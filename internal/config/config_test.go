...
func TestValidateAcceptsMaximumToolCallTimeout(t *testing.T) {
	c := valid()
	c.Limits.ToolCallTimeout = time.Duration(1<<63 - 1)
	if err := c.Validate(); err != nil {
		t.Fatalf("maximum tool_call_timeout rejected: %v", err)
	}
}
